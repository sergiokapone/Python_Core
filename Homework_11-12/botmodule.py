...
class AddressBook(UserDict):
    ...
    def get_birthdays_per_week(self):
        """Метод визначає іменинників на тижні"""
        ...
        for name, record in self.items():
            ...
            user_birthday = (
                datetime.strptime(birthday, "%d.%m.%Y").replace(year=today.year).date()
            )

            if user_birthday < week_start - timedelta(days=2):
                user_birthday = user_birthday.replace(year=today.year + 1)

            if (
                week_start - timedelta(days=2)
                <= user_birthday
                <= week_end - timedelta(days=2)
            ):
                weekday = (
                    user_birthday.strftime("%A")
                    if user_birthday.weekday() < 5
                    else "Monday"
                )

                birthdays.setdefault(weekday, []).append(name)
    ...
    def import_from_csv(self, filename):
        ...
        with open(filename, newline="") as csv_file:
            ...
            for row in reader:
                ...
                birthday = Birthday(row["birthday"]) if row["birthday"] != "-" else None
